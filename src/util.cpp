...
const char * const BITCOIN_CONF_FILENAME = "utharncoin.conf";
const char * const BITCOIN_PID_FILENAME = "utharncoind.pid";
...
#else
    const char* pszModule = "utharncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UtharnCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UtharnCoin";
#else
    // Unix
    return pathRet / ".utharncoin";
#endif
#endif
}
