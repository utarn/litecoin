 Core version 0.13.2 is now available from:

  <https://download..org/-0.13.2.1/>
...
ECDSA signatures inside  transactions now use validation using
...
Previous versions of  Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of  Core was to raise the default minimum
relay fee.

 Core 0.13.2 will have a strict maximum size on the mempool. The
...
1000 satoshis per kB.
 ...
 Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running d without having to do any manual configuration.
...
---------------------------------------
 ...
 Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks.  Core will relay transactions with
...
entirely in  Core 0.13, and it is at this time undecided whether the
...
API, to create and destroy 'ephemeral' hidden services programmatically.
 Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
 Core automatically creates a hidden service to listen on, without
manual configuration.  Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if  Core is listening, and
...
d can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals 
Core to use floating fees. By default,  Core will use floating
fees.
...
At all times,  Core will cap fees at `-maxtxfee=<x>` (default:
0.10) .
Furthermore,  Core will never create transactions paying less than
...
caching. A sample config for apache2 could look like:
    ...
    <Location /rpc>
        ...
        # optional bypass d rpc basic auth
...
Restarting d will no longer clear out the list of banned peers; instead
...
For example:
    ...
    $ src/-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The  Core code base has
...
- `-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the  scripting system that in combination with
...
a consensus rule.
 ...
 transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of  Core.
...
The two phases are distinct in the -Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming  wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each  block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:**  addresses (both P2PKH addresses
  ...
  believed to be provided by 's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of 's current ledger (called the UTXO set),
  ...
  note that  Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
   users to individually opt-in to almost any change in the 
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and  Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the 
client to  Core 0.13.2 to signal for segwit support. When a
...
Since  Core 0.10.0, nodes have defaulted to only relaying and
...
- `-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
